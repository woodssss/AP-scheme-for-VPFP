function PN_search_VPFP_1D(epsi,Nx,Nv,Lx,Lv,tau,T,opt1)
%%%%%%%%%%%%% Proximal Quasi newton method for VPFP %%%%%%
...
rho_x0 = (2*pi)^0.5*(2+cos(2*pi*x));
h_x = 5.0132/1.2661*exp(cos(2*pi*x));
if nargin<8
    opt1=0;
...
for i = 1 :Nx
    for j = 1:Nv
        ...
        f0(j,i) = rho_x0(i)/(2*pi)^0.5*(exp(-(v(j)+1.5)^2/2)+exp(-(v(j)-1.5)^2/2))/2;
    end
end
t=0;
f=f0;
...
while t < tfinal
    ...
    for i = 1:Nx
        for j = 1:Nv
            M(j,i) = exp(-(v(j)+grad_theta_x(i))^2/2)/(2*pi)^0.5;
        end
    end
    for i = 1:Nx
        M(:,i) = rho(i)*M(:,i);
    ...
    %% Collision
    parfor i = 1:Nx
        f(:,i) = PN(f(:,i),M(:,i),rho(i),epsi,Nv,A,tau,dv);
    end
    if opt1 ==1
        for i = 1:Nx
    ...
    t = t+dt
    count=count+1;
    E2M(1,count) = sum(sum(abs(f-M)))*dx*dv;
    if rem(t,0.1)<1e-5
        for i = 1:Nx
            rho_new(i)=sum(f(:,i))*dv;
        end
        plot(x,rho_new)
        drawnow
        filename=['PN_vpfp_epsi', num2str_decimal(epsi),'_t_'   ,  num2str_decimal(t)];
        save(filename)
        disp(['t=',num2str(t)])
    end
end
...
for k = 1:maxiter
    ...
    tk=1;
    while abs(imag((energyy(u+tk*vv,M,epsi,tau,dv,Nv))))>1e-20  || energyy(u+tk*vv,M,epsi,tau,dv,Nv)>energyy(u,M,epsi,tau,dv,Nv)+0.01*tk*grad(u,M,epsi,Nv,tau)'*vv  %|| norm(A*(u+tk*v)-b)>1e-6
        tk=0.5*tk;
    end
    u_next=u+tk*vv;
