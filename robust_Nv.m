...
for ii = 1:NN
    Nv = 64*2^(ii-1);
    PN_1D_end(1,Nv,L,0.5,tau,tau,IC);
    filename = ['PN_tau_', num2str_decimal(tau), '_T_', num2str_decimal(tau), '_epsi_',num2str_decimal(1),'_end'];
    load(filename)
    u_star=u;
    for j = 1:KKK
...
for ii = 1:NN
    Nv = 64*2^(ii-1);
    PN_1D_end(1e-5,Nv,L,0.4,tau,tau,IC);
    filename = ['PN_tau_', num2str_decimal(tau), '_T_', num2str_decimal(tau), '_epsi_',num2str_decimal(1e-5),'_end'];
    load(filename)
    u_star=u;
    for j = 1:KKK
...
figure(1)
semilogy(xk,error_u_epsi_1(1,:),'k-p',xk,error_u_epsi_1(2,:),'r-o',xk,error_u_epsi_1(3,:),'b.-',xk,error_u_epsi_1(4,:),'Linewidth',2)
...
figure(2)
semilogy(xk,error_u_epsi_n5(1,:),'k-p',xk,error_u_epsi_n5(2,:),'r-o',xk,error_u_epsi_n5(3,:),'b.-',xk,error_u_epsi_n5(4,:),'Linewidth',2)
