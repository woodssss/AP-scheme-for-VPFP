function diff_outer_step(Nv,L,tau,T)
%%%%%%%%%%%%% test convergence at different time steps %%%%%%
% Use fix step size method
...
% Code for Figure 4
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
PN_1D_end(1,Nv,L,0.5,tau,T,3);
filename = ['PN_tau_', num2str_decimal(tau), '_T_', num2str_decimal(T), '_epsi_',num2str_decimal(1),'_end'];
load(filename)
NN=4;
KKK=150;
...
PN_1D_end(1e-5,Nv,L,0.4,tau,T,3);
filename = ['PN_tau_', num2str_decimal(tau), '_T_', num2str_decimal(T), '_epsi_',num2str_decimal(1e-5),'_end'];
load(filename)
NN=4;
KKK=150;
Nstep=round((T-tau)/tau/NN);
