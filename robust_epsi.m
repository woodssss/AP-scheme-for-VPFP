function robust_epsi(Nx,L,tau,IC)
%%%%%%%%%%%%% Robustness to epsi %%%%%%
% Use fix step size
...
for ii = 1:NN
    ...
    PN_1D_end(temp_eps,Nx,L,lambda,tau,tau,IC);
    filename = ['PN_tau_', num2str_decimal(tau), '_T_', num2str_decimal(tau), '_epsi_',num2str_decimal(temp_eps),'_end'];
    load(filename)
    u_star=u;
    f_star=f;
    for j = 1:KKK
