...
for ii = 1:NN
    temp_tau = 0.4/2^(ii-1);
    PN_1D_end(1,Nx,L,0.5,temp_tau,temp_tau,IC);
    filename = ['PN_tau_', num2str_decimal(temp_tau), '_T_', num2str_decimal(temp_tau), '_epsi_',num2str_decimal(1),'_end'];
    load(filename)
    u_star=u;
    for j = 1:KKK
...
for ii = 1:NN
    temp_tau = 0.4/2^(ii-1);
    PN_1D_end(1e-5,Nx,L,0.4,temp_tau,temp_tau,IC);
    filename = ['PN_tau_', num2str_decimal(temp_tau), '_T_', num2str_decimal(temp_tau), '_epsi_',num2str_decimal(1e-5),'_end'];
    load(filename)
    u_star=u;
    for j = 1:KKK
