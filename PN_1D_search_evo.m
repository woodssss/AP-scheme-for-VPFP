function PN_1D_search_evo(epsi,Nx,L,tau,T,IC)
...
while t<T
    ...
    for k = 1:maxiter
        ...
        tk=1;
        while abs(imag((energyy(u+tk*v,tau,dx,M,epsi,Nx))))>1e-25  || energyy(u+tk*v,tau,dx,M,epsi,Nx)>energyy(u,tau,dx,M,epsi,Nx)+0.002*tk*grad(u,epsi,M,Nx,tau)'*v  %|| norm(A*(u+tk*v)-b)>1e-6
            tk=0.5*tk;
        end
        u_next=u+tk*v;
        u=u_next;
...
drawnow
%filename='PN_search_1D_dx16_n5_tauzz5';
%save(filename)
